...
import { ADD_PRODUCT_URL } from "../conf/index";
...
class AddProduct extends Component {
	...
	formSubmit = async () => {
		const { name, description, price, selectedFile } = this.state;
		...
		if (selectedFile === null) {
			NotificationManager.error("Product photo should be filled.", "Error!", 5000);
		} else if (name === "") {
			NotificationManager.error("Product name should be filled.", "Error!", 5000);
		} else if (description === "") {
			NotificationManager.error("Product description should be filled.", "Error!", 5000);
		} else if (price_temp === 0) {
			NotificationManager.error("Product price cannot be 0.", "Error!", 5000);
		} else {
			await axios.post(`${ADD_PRODUCT_URL}`, formData, {}).then(res => {
				if (!res.data.error) {
					this.handleBackHome();
					this.setState({ open: true, result: "success" });
					NotificationManager.success("New product has been added successfully.", "Success!", 5000);
				} else {
	...
	render() {
		...
		const { fileUrl, name, description, price } = this.state;
